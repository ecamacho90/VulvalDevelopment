function sol=simulationEulerACablation_Competence_v10(t0,t1,tAC,dt,dtPC,M,MPC,parametersmodel,nsimulations,Z0D,Z1D,Z2D,Z3D,Z4D,Z5D,Z6D,Z1DPC,Z2DPC,Z3DPC,Z4DPC,Z5DPC,Z6DPC,aPC,bPC,cPC,DiscriminantPC,equilibriax,saddlefate3)
...
modeleqstr='cusp_and_saddlenode_model_v10';
...
if (tAC>=t1)  %If tAC is greater or equal than t1  there is no AC ablation, thus we compute the fates with respect to the normal parameters
    ...
    param(34) = 0; %Good because paramPC(27)=0;
      ...
      Xprev=Z0D(:,simulation);    %Set the initial point for each cell at the new simulation
      ...
      %%%%%%%%%%%%%%%%%%%%%
        for timestep=1:steps
            ...
            Xprev=Xprev+dtfprev+[Z1D(simulation,timestep);Z2D(simulation,timestep);Z3D(simulation,timestep);Z4D(simulation,timestep);Z5D(simulation,timestep);Z6D(simulation,timestep)];  %Renew the state taking an Euler Step
...
%           end
      ...
      %%%%%%%%%%%%%%%%%%%%%
        ...
        for timestep = 1:stepsPC
            ...
            Xprev=Xprev+dtfprev+[Z1DPC(simulation,timestep);Z2DPC(simulation,timestep);Z3DPC(simulation,timestep);Z4DPC(simulation,timestep);Z5DPC(simulation,timestep);Z6DPC(simulation,timestep)];  %Renew the state taking an Euler Step
      ...
      %%%%%%%%%%%%%%%%%%%%%    
        ...
        if norm(Xprev)<10^10
            fatessimulated = computefates_PostCompetence(Xprev,param(2),param(3),aPC,bPC,cPC,DiscriminantPC,saddlefate3,equilibriax);  %Compute the fates corresponding to the end point of the simulated trajectory after competence
        else
